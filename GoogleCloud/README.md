...
![](Images/nosi-ua-architecture.svg)
...
To create a virtual machine for GCP, follow steps 1-8 of [these instructions]([https://github.com/STRIDES/NIHCloudLabGCP/blob/main/docs/vertexai.md](https://github.com/NIGMS/NIGMS-Sandbox/blob/main/docs/HowToCreateVertexAINotebooks.md) and be especially careful to enable idle shutdown as highlighted in step 8. For this module, the `n1-standard-4` machine type will be sufficient. Additionally, a GPU is required for submodules 1-3 to substantially decrease network training times, and thus a single `NVIDIA T4` GPU is recommended. When selecting a GPU, you will also want to ensure that `Install NVIDIA GPU driver automatically for me` is checked.
...
Generative is available in the form of Gemini if you would like to use it there. To run it, please reference the 01-Classification notebook or run the following code. You will need to save the util folder with the gemini.py file in the same directory as the notebook where you are running Gemini.
